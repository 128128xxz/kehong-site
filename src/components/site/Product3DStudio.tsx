      ...
      <div className="mx-auto grid max-w-[90rem] gap-8 lg:grid-cols-[.7fr_1.3fr] lg:items-center lg:gap-16">
        <div>
          ...
          <h2 className="kh-editorial-heading mt-4 max-w-xl text-4xl leading-[.98] tracking-[-.045em] sm:text-6xl">
            {isZh ? "投产前确认包装结构。" : "Review the structure before production."}
          </h2>
          <p className="mt-5 max-w-md text-base leading-7 text-[#626156]">
            {isZh
              ? "先检查面板关系、折叠顺序和材料层次，再进入独立 3D 展厅查看完整模型。"
              : "Inspect panel relationships, fold sequence, and material layers before opening the full model in the dedicated 3D Studio."}
          </p>
        ...
        <Link href="/model-preview" aria-label={isZh ? "打开 3D 包装结构展厅" : "Open the 3D packaging structure studio"} className="kh-3d-technical group relative grid overflow-hidden border border-[#c9bea9] bg-[#ebe6dc] lg:grid-cols-[7fr_3fr]">
          <div className="kh-3d-technical__model relative min-h-0 overflow-hidden">
            <ResilientImage
              src={showcaseImages.modelTechnicalPreview}
              fallbackSrc={showcaseImages.webOpenBox}
              alt={isZh ? "纸盒、内托和纸杯结构的三维技术预览" : "3D technical preview of paper box, insert, and cup structures"}
              fill
              loading="eager"
          ...
          <div className="kh-3d-technical__detail flex flex-col border-t border-[#c9bea9] bg-[#162d27] text-white lg:border-l lg:border-t-0">
            <div className="relative min-h-0 flex-1 overflow-hidden border-b border-white/12">
              <ResilientImage
                src={showcaseImages.structureMaterialReal}
                fallbackSrc={showcaseImages.webCorrugatedSheet}
                alt={isZh ? "瓦楞纸板材料层次细节" : "Corrugated board material-layer detail"}
                fill
                loading="lazy"
                sizes="(min-width: 1024px) 20vw, 45vw"
                className="object-cover opacity-72"
              />
              <div className="absolute inset-0 bg-gradient-to-t from-[#102820]/92 via-[#102820]/26 to-transparent" />
              <p className="absolute bottom-3 left-3 right-3 flex items-center gap-2 text-xs font-black uppercase tracking-[.14em] text-[#f2d080]">
            ...
            <div className="kh-3d-checks p-4">
              ...
              <ul className="mt-3 grid gap-2 text-xs font-semibold text-white/74">
                <li><span>01</span> Open / closed state</li>
