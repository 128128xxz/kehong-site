...
function SolutionVisual({ solution, copy }: { solution: (typeof solutions)[number]; copy: (typeof solutions)[number]["en"] | (typeof solutions)[number]["zh"] }) {
  if (solution.visual === "photo") {
    return <ResilientImage src={showcaseImages.foodOpen} fallbackSrc={showcaseImages.foodDetail} alt={copy.title} fill sizes="(min-width: 1024px) 50vw, 94vw" className="object-cover object-[18%_16%] opacity-72 transition duration-700 group-hover:scale-[1.04] group-hover:opacity-84" />;
  }
