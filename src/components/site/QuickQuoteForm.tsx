...
type ProductSeed = { sku?: string; name?: string; url?: string };
...
export default function QuickQuoteForm({ locale, initialProducts = [] }: { locale: string; initialProducts?: ProductSeed[] }) {
  ...
  const initialProductText = useMemo(
    () => [...initialProducts, ...storedProducts].map((item) => [item.sku, item.name, item.url].filter(Boolean).join(" | ")).filter(Boolean).join("\n"),
  ...
  return (
    <form onSubmit={submit} className="text-(--kh-ink)" noValidate>
      ...
      <label className="mt-4 flex items-start gap-2 text-xs leading-5 text-(--kh-muted)">
        ...
        <span>{zh ? <>我同意 Kehong 按照 <Link href="/privacy" className="kh-inline-link">隐私政策</Link> 处理本次询盘。</> : <>I agree that Kehong may process this inquiry under the <Link href="/privacy" className="kh-inline-link">Privacy Policy</Link>.</>}</span>
      </label>
...
function FileField() {
  return <label className="flex min-h-[5.25rem] items-center gap-3 rounded-md border border-dashed border-(--kh-brass)/55 bg-(--kh-paper) px-3 text-sm font-semibold text-(--kh-muted) sm:col-span-2"><Upload className="size-5 shrink-0 text-(--kh-brass)" /><span className="flex-1">Reference image, dieline or brief <span className="font-normal">(optional)</span></span><input name="attachment" type="file" accept="image/*,.pdf,.doc,.docx,.xls,.xlsx" className="max-w-[10.5rem] text-xs font-normal" /></label>;
}
