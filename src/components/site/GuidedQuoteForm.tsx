...
const copy = {
  en: {
    ...
    print: "Printing & finish", colors: "Printing colors", colorsPlaceholder: "CMYK / Pantone / reference", printSide: "Inside or outside printing", printSidePlaceholder: "Outside / inside / both", finish: "Lamination / foil / emboss / coating", finishPlaceholder: "Finish direction", artwork: "Artwork readiness", artworkPlaceholder: "Ready / in progress / unsure", upload: "Artwork or reference file", uploadNote: "File upload status is shown by the browser. Large or unsupported files may need to be shared after the first inquiry.",
  ...
  zh: {
    ...
    print: "印刷与后处理", colors: "印刷颜色", colorsPlaceholder: "CMYK / Pantone / 参考图", printSide: "内侧或外侧印刷", printSidePlaceholder: "外侧 / 内侧 / 双面", finish: "覆膜 / 烫印 / 压纹 / 涂层", finishPlaceholder: "后处理方向", artwork: "设计文件状态", artworkPlaceholder: "已准备 / 制作中 / 暂不确定", upload: "设计文件或参考资料（可选）", uploadNote: "浏览器会显示文件选择状态。较大或不支持的文件可在首次询盘后补充。",
    delivery: "交付与联系信息", destination: "目的国家 / 邮编", shipping: "运输方式 / 贸易术语", company: "公司", companyPlaceholder: "公司名称", name: "姓名", namePlaceholder: "您的姓名", email: "邮箱", phone: "电话 / WhatsApp", optional: "可选", notes: "备注", notesPlaceholder: "交期、目的地或其他项目说明", privacyPrefix: "我同意科宏纸品根据", privacy: "隐私政策处理我提交的信息，以便回复本次询盘",
  },
} as const;
...
export default function GuidedQuoteForm({ locale, initialProducts = [] }: GuidedQuoteFormProps) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!String(data.get("name") ?? "").trim() || !String(data.get("email") ?? "").trim() || !productText || data.get("privacy") !== "on") { setError(text.required); setStatus("error"); setStep(4); return; }
    setStatus("sending"); setError("");
    try { const interest = initialProducts.find((item) => item.interestId) ?? initialProducts[0]; if (interest?.interestId) data.set("interestId", interest.interestId); if (interest?.interestLabel) data.set("interestLabel", interest.interestLabel); if (interest?.interestProductType) data.set("interestProductType", interest.interestProductType); data.set("productGroupId", interest?.productGroupId ?? ""); data.set("productGroupTitle", interest?.productGroupTitle ?? interest?.name ?? ""); data.set("sku", interest?.sku ?? ""); data.set("skuTitle", interest?.skuTitle ?? ""); data.set("products", productText); data.set("sourceUrl", window.location.href); appendAttribution(data, "guided_quote", locale); const response = await fetch("/api/inquiry", { method: "POST", body: data }); if (!response.ok) throw new Error("request failed"); setStatus("success"); trackKehongEvent("inquiry_submit", { locale, ctaLocation: "guided_quote", productGroupId: interest?.productGroupId }); form.reset(); } catch { setStatus("error"); setError(text.error); }
  }
