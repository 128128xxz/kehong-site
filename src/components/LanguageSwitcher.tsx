...
const LanguageSwitcher = () => {
  ...
  const switchLocale = (locale: (typeof routing.locales)[number]) => {
    ...
    // `document.cookie` is an accessor property. Use Reflect.set so the
    // browser still performs the normal cookie write without triggering the
    // immutability rule for direct assignments to DOM globals.
    Reflect.set(document, "cookie", `NEXT_LOCALE=${locale}; Max-Age=31536000; Path=/; SameSite=Lax${secure}`);
    Reflect.set(document, "cookie", `kehong_locale=${locale}; Max-Age=31536000; Path=/; SameSite=Lax${secure}`);
