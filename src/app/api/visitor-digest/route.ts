...
import { digestDayToSend, digestWindowToSend } from "@/server/b2b-intelligence/service";
...
export async function GET(request: Request) {
  ...
  if (kind !== "noon" && kind !== "evening" && kind !== "daily" && kind !== "all") return NextResponse.json({ ok: false, code: "WINDOW_REQUIRED" }, { status: 400 });
  ...
  const now = new Date();
  const reportDay = kind === "daily" ? digestDayToSend(now) : undefined;
  const digestWindow = kind === "all" ? "all-retained" : kind === "daily" ? `daily-${reportDay}` : digestWindowToSend(now, kind as DigestWindowKind);
  ...
  try {
    const rows = kind === "all"
      ...
      : kind === "daily"
        ? await store.getDayVisitors(reportDay!, now.toISOString())
        : await store.getWindowVisitors(digestWindow, now.toISOString());
