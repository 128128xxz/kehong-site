  ...
  return (
    <div className="texture-paper min-h-screen text-(--kh-ink)">
      ...
      <main className="mx-auto max-w-4xl px-4 py-14 sm:px-6 lg:px-8">
        <p className="kh-eyebrow">{isZh ? "法律信息" : "Legal information"}</p>
        <h1 className="kh-editorial-heading mt-4 text-4xl text-(--kh-ink)">{isZh ? "使用条款" : "Terms of Use"}</h1>
        <div className="prose prose-stone mt-8 max-w-none">
          <p>{isZh ? "本网站提供纸品包装材料、半成品与成品结构的产品信息，具体规格、价格、交期和可用性需以双方确认的报价和订单为准。" : "This website provides information about paper packaging materials, components and finished structures. Final specifications, pricing, lead time and availability are confirmed in the quotation and order agreed by both parties."}</p>
          <h2>{isZh ? "产品信息" : "Product information"}</h2>
          <p>{isZh ? "图片用于展示材料或结构方向，除非明确标注为精确产品图片，否则不应视为特定 SKU 的承诺。" : "Images may illustrate a material or structure direction. Unless explicitly identified as exact product photography, they are not a commitment for a specific SKU."}</p>
