...
import { getR2HeroForPackagingRoute } from "@/data/r2WebsiteAssets";
...
const allPackagingCategories: PackagingCategory[] = [
  {
    ...
    description: { en: "Develop a paper bag around your product, handle style and brand finish. Kehong reviews paper grade, dimensions, printing, surface treatment and packing requirements from the packaging details you provide.", zh: "纸袋按产品类型、手柄形式和品牌工艺开发。科宏根据你提供的包装需求评估纸张、尺寸、印刷、表面处理和包装要求。" },
    image: getR2HeroForPackagingRoute("paper-bags")?.image ?? showcaseImages.aiPaperBagBranded,
  ...
  },
  {
    ...
    image: getR2HeroForPackagingRoute("cake-boxes")?.image ?? showcaseImages.aiCakeBoxWindow,
  ...
  },
  {
    ...
    image: getR2HeroForPackagingRoute("cake-boards-cake-drums")?.image ?? showcaseImages.aiCakeBoardsSet,
  ...
  },
  {
    ...
    image: getR2HeroForPackagingRoute("pizza-packaging")?.image ?? showcaseImages.aiPizzaBox,
  ...
  },
  {
    ...
    description: { en: "Select the food format first, then review the footprint, board, barrier, closure, ventilation, printing and packing workflow. Kehong can review boxes, trays, pads and related paper components as one packaging requirement.", zh: "先确认食品应用，再评审尺寸、纸板、阻隔、闭合、通风、印刷和包装流程。科宏可将食品盒、纸托、垫纸及相关纸材部件作为一项整体包装需求评审。" },
    image: getR2HeroForPackagingRoute("food-packaging")?.image ?? showcaseImages.takeoutBoxesReference,
  ...
  },
  {
    ...
    image: getR2HeroForPackagingRoute("inserts-dividers")?.image ?? showcaseImages.representativeInserts,
  ...
  },
  {
    ...
    image: getR2HeroForPackagingRoute("retail-packaging")?.image ?? showcaseImages.retailShelfDisplay,
  ...
  },
  {
    ...
    image: getR2HeroForPackagingRoute("corrugated-mailer-boxes")?.image ?? showcaseImages.representativeProtectiveStructures,
